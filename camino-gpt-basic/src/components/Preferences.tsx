...
import {
    UNIT_SYSTEMS, ROUTE_STYLES, BUDGET_TIERS, ALBERGUE_KINDS,
    DIETARY_OPTIONS, LANGUAGE_OPTIONS, LABELS
                    ...
                    <div className="w-[380px] max-w-[95vw] bg-[#151515] border-l border-[#262626] p-3 overflow-y-auto">
                        ...
                        <Section title="Units">
                            <Radio
                                ...
                                options={UNIT_SYSTEMS.map((u) => [u, LABELS.unitSystem[u]] as [string, string])}
                        ...
                        <Section title="Route Style">
                            <Radio
                                ...
                                options={ROUTE_STYLES.map((r) => [r, LABELS.routeStyle[r]] as [string, string])}
                        ...
                        <Section title="Budget">
                            <Radio
                                name="budget"
                                value={prefs.budget}
                                options={BUDGET_TIERS.map((b) => [b, LABELS.budget[b]] as [string, string])}
                                onChange={(v) => update("budget", v as any)}
                            />
                        </Section>
                        ...
                        <Section title="Stays">
                            <Checks
                                ...
                                options={ALBERGUE_KINDS.map((k) => [k, LABELS.albergueKinds[k]] as [string, string])}
                        ...
                        <Section title="Dietary">
                            <Checks
                                value={prefs.dietary}
                                options={DIETARY_OPTIONS.map((d) => [d, LABELS.dietary[d]] as [string, string])}
                                onToggle={(v) => {
                                    if (v === "none") return update("dietary", ["none"]);
                                    const base = prefs.dietary.includes("none") ? [] : prefs.dietary;
                                    update(base.includes(v as any) ? "dietary" : "dietary",
                        ...
                        <Section title="Language Help">
                            <Checks
                                ...
                                options={LANGUAGE_OPTIONS.map((l) => [l, LABELS.language[l]] as [string, string])}
...
function Radio({
    ...
    name: string; // ensure radios in different sections don't share the same name
    value: string;
    ...
    return (
        <div className="flex gap-2 flex-wrap">
            {options.map(([val, label]) => (
                <label
                    key={val}
                    className={`cursor-pointer rounded-xl px-3 py-1 border ${value === val ? "border-white" : "border-[#2a2a2a]"
                        }`}
                >
                    <input
                        type="radio"
    ...
    return (
        <div className="flex gap-2 flex-wrap">
            {options.map(([val, label]) => (
                <label
                    key={val}
                    className={`cursor-pointer rounded-xl px-3 py-1 border ${value.includes(val) ? "border-white" : "border-[#2a2a2a]"
                        }`}
                >
                    <input
                        type="checkbox"
